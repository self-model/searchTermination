...
group_df = to_csv(path.join('..','experiments','pilot_shape','data','jatos_results_batch1'))
...
df <- read_csv('../experiments/pilot_shape/data/jatos_results_batch1.csv') %>%
...
median_search_times <- search_df %>%
  filter(include==1) %>%
  group_by(response,search_type,set_size) %>%
  summarise(mean_trial=mean(trial),median_RT= median(RTfiltered), 
